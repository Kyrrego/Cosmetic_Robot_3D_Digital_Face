...
static bool saveFrameData(const char* saveDir,cs::IFramePtr frame, int index)
{
	...
	{
		snprintf(acPath, sizeof(acPath), "%s/rgb%dx%d-%d-%d.bmp",saveDir, frame->getWidth(), frame->getHeight()
			, index, frame->getSize());
	...
	{
		snprintf(acPath, sizeof(acPath), "%s/rgb%dx%d-%d.jpg", saveDir, frame->getWidth(), frame->getHeight()
			, index);
	...
	{
		snprintf(acPath, sizeof(acPath), "%s/z16-%dx%d-%d-%d.bmp", saveDir, frame->getWidth(), frame->getHeight()
			, index, frame->getSize());
		cs::colorizer color;
	...
	else if (frame->getFormat() == STREAM_FORMAT_Z16Y8Y8)
	{
		...
		snprintf(acPath, sizeof(acPath), "%s/z16y8y8-%dx%d-%d-%d.bmp", saveDir, frame->getWidth(), frame->getHeight()
			, index, frame->getSize());
		cs::colorizer color;
		...
		snprintf(acPath, sizeof(acPath), "%s/depth-%d-%dx%d-%d-%d_y8-left.bmp", saveDir, frame->getFormat()
		...
		snprintf(acPath, sizeof(acPath), "%s/depth-%d-%dx%d-%d-%d_y8-right.bmp", saveDir, frame->getFormat()
			, frame->getWidth(), frame->getHeight()
			, index, frame->getSize());
	...
	{
		snprintf(acPath, sizeof(acPath), "%s/pair-%dx%d-%d-%d.bmp", saveDir, frame->getWidth(), frame->getHeight()
			, index, frame->getSize());
		int iLen = frame->getWidth()*frame->getHeight() * 2;
		if (frame->getSize() != iLen)
		{
		...
		cs::saveBmp((const char *)frame->getData()
			, frame->getWidth() * 2, frame->getHeight(), 1, acPath);
		printf("save Ok:%s\n", acPath);
	}
	else
	{
